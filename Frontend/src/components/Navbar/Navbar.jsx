    ...
    <>
      <nav className="z-50 relative flex bg-zinc-800 px-8 py-4 text-white items-center justify-between">
        ...
        <div className="nav-links-bookheaven block md:flex items-center gap-4">
          ...
          <button
            className="block md:hidden text-white text-2xl hover:text-zinc-400 on"
            onClick={() =>
              MobileNav === "hidden"
                ? setMobileNav("block")
                : setMobileNav("hidden")
      ...
      >
        {links.map((items, i) => (
          <Link
            to={items.link}
            className={`${MobileNav} mb-4 text-white text-4xl font-semibold hover:text-blue-500 transition-all duration-300`}
            key={i}
            onClick={() =>
              MobileNav === "hidden"
                ? setMobileNav("block")
